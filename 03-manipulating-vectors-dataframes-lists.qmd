...
adsl3 <- select(adsl, -c(STUDYID:ARM))
adsl4 <- select(adsl, -c(1:6))
adsl4 <- select(adsl, -c(1:6, 43))
adsl5 <- select(adsl, -starts_with("A"))
adsl6 <- select(adsl, -ends_with("DTC"))
adsl7 <- select(adsl, -contains("TRT"))
```
...
adsl1 <- adsl |> 
  ...
  dplyr::select(USUBJID, AGE, age_group) |> 
  head(10)
```
# create seq no.
```{r}
